...
std::pair<Grid2D, int> conjugate_gradient(const Grid2D& U0, const Grid2D& F, const BoolGrid2D& mask,
    ...
    int num_unknowns = 0;
    #pragma omp parallel for collapse(2) reduction(+:num_unknowns)
...
int main(int argc, char* argv[]) {
    if (argc != 4) {
        std::cout << "Использование: " << argv[0] << " <M> <N> <num_threads>" << std::endl;
        std::cout << "Пример: " << argv[0] << " 40 40 4" << std::endl;
        return 1;
    }
    ...
    std::pair<Grid2D, int> result = conjugate_gradient(U, F_rhs, mask, border_mask, 
                                       a_coeffs, b_coeffs, hx, hy, (N-1) * (M-1), 1e-8, num_threads);
    double end_time = omp_get_wtime();
    double time = end_time - start_time;
    
    std::cout << "Время решения: " << time << " с" << std::endl;
    
    std::string filename_csv = "solution_" + to_string(M) + "_" + to_string(N) + "_" + to_string(num_threads) + "_omp.csv";
    save_solution_csv(X, Y, result.first, mask, filename_csv);

    std::string filename_txt = "solution_" + to_string(M) + "_" + to_string(N) + "_" + to_string(num_threads) + "_omp.txt";
