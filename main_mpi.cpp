...
std::pair<int, int> find_optimal_2d_decomposition(int M, int N, int size) {
    ...
    for (int px = 1; px <= size; ++px) {
        ...
        // Проверяем условие 3: разница не более 1
        if (M_max - M_min > 1 || N_max - N_min > 1) continue;
        
        // Проверяем условие 2: отношение узлов в [1/2, 2] для всех доменов
...
double norm_2d(const Grid2D& u, const BoolGrid2D& mask, const ProcessTopology2D& topo) {
    return std::sqrt(dot_product_2d(u, u, mask, topo));
}
...
std::pair<Grid2D, int> conjugate_gradient_2d(const Grid2D& U0, const Grid2D& F, const BoolGrid2D& mask,
                         ...
                         int max_iter, double tol, const ProcessTopology2D& topo) {
    ...
    Grid2D Aw0 = apply_A_2d(w, mask, border_mask, a_coeffs, b_coeffs, hx, hy, topo);
    ...
    Grid2D z = solve_D_2d(r, mask, border_mask, a_coeffs, b_coeffs, hx, hy, topo);
    Grid2D p = z;
    
    double rz_old = dot_product_2d(r, z, inner_mask, topo);
    ...
    for (int k = 0; k < max_iter && k < global_num_unknowns; ++k) {
        ...
        // Вычисляем невязку
        double current_residual = norm_2d(r, inner_mask, topo);
        ...
        Grid2D Ap = apply_A_2d(p, mask, border_mask, a_coeffs, b_coeffs, hx, hy, topo);
        double pAp = dot_product_2d(p, Ap, inner_mask, topo);
        ...
        exchange_boundaries_2d(w, topo, exchange_buffers, MPI_COMM_WORLD);
        z = solve_D_2d(r, mask, border_mask, a_coeffs, b_coeffs, hx, hy, topo);
        double rz_new = dot_product_2d(r, z, inner_mask, topo);
...
int main(int argc, char* argv[]) {
    ...
    std::pair<Grid2D, int> result = conjugate_gradient_2d(U, F_rhs, mask, border_mask, 
                                       a_coeffs, b_coeffs, hx, hy, (N-1) * (M-1), 1e-8, topo);
    ...
    if (rank == 0) {
        ...
        std::string filename_csv = "solution_" + int_to_string(M) + "_" + int_to_string(N) + "_" + int_to_string(size) + "_mpi_2d.csv";
        save_solution_csv(global_X, global_Y, global_U, global_mask, filename_csv);
        
        std::string filename_txt = "solution_" + int_to_string(M) + "_" + int_to_string(N) + "_" + int_to_string(size) + "_mpi_2d.txt";
